...
users can build fancy kibana dashboards by extracting metrics from elastic-search queries.

With the development of micro-services architectures, the number of emitted logs recently exploded
making these ELK very hardware and therefore money consuming. Even more, theses stack are often
...
`{path}` depends on parsed [Structured Data ID][structured-data] fields of log the event.
...
- Clusterize multiple concentrators in order the create a sort of backup across independent BOSH
  directors
...
- `<targets>`: a list of syslog endpoint where matching events are forwarded. When multiple
...
Assuming that yours deployment uses [bosh-dns], the job `loghost_dns` can be used to define new
aliases.
...
- `LoghostNoLogReceived`: triggers if exported reports no processed logs in the last 15 minutes

When `loghost_alerts.enable_security` key is set to true (default false), the job also defines
the following alerts:
- `SecurityTooManyAuthFailures`: triggers when `audispd` logs reports too many auth failures.
...
- a security dashboard overview giving informations on authentications when
  `loghost_dashboards.enable_security` key is enabled.
